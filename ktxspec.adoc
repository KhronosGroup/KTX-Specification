...
for each mipmap_level in numberOfMipmapLevels <1>
    ...
    Byte mipPadding[7 - ((imageSize + 7) % 8)]
end
----
...
`dataCompressionScheme == 0` or after expansion when
...
all `keyAndValueByteSize` fields, all `keyAndValue` fields, all
`valuePadding` fields and the `keyValuePadding` field. The file
offset of the `totalDataSize` field is located at the file
offset of the `bytesOfKeyValueData` field plus the value of the
...
following `totalDataSize` field is at a file offset that is a multiple
of 8. This padding is included in the `bytesOfKeyValueData` field.
...
=== mipPadding
Between 0 and 7 bytes to make sure that all `imageSize` fields are
at a file offset that is a multiple of 8.
