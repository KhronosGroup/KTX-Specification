...
== General comments
Uncompressed pixel data must match the default OpenGL unpack
alignment: 4. I.e. the data must be packed according to the rules
described in section 8.4.4.1 of the OpenGL 4.6 specification
<<OPENGL46>> with UNPACK_ROW_LENGTH = 0 and UNPACK_ALIGNMENT = 4.
...
Should Global Compression be per-mip-level compression?::
  ...
  be a level index to enable random access. The overall effect on the data
  size is unclear.
+
_Resolved:_ Yes. The benefits of random access, such as streaming,
outweigh what is expected to be a small increase in size.
