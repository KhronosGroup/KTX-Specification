...
+++++
+
`block_depth`, `block_height`, and `block_width` are `1` for uncompressed
formats and the block size in that dimension for block compressed formats
as given in that format's section of the Khronos Data Format specification <<KDF12>>.
+
A `block` is a single pixel for uncompressed formats and
+
[stem]
+++++
"block_width" xx "block_height" xx "block_depth"
+++++
+
...
The value of `bytesOfUncompressedLevelImages` must satisfy the following condition.
[listing]
