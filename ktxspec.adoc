...
== General comments
=== Endianness
KTX files are little endian. All header fields and all uncompressed
XXX Is compressed data little-endian as well? XXX
data is stored in little endian order. Readers on big-endian machines
must endian convert all header UInt32s and UInt64s and, when the
data is uncompressed XXX does the DFD test for component type size
result in a size of 1 for compressed formats? XXX and `<<glTypeSize>> != 1`, all `data` to the
big endian and writers must endian convert these items to little
endian.
