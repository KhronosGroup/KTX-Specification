...
[NOTE]
====
Specifications which refer to KTX2 as their base format may wish to
restrict the `color_primaries` that can be used.
...
* If `<<_vkformat,vkFormat>>` is one of the `+*_SRGB{,_*}+` formats then
  `transfer_function` must be `KHR_DF_TRANSFER_SRGB`. This requirement
  is bidirectional. If `transfer_function` is `KHR_DF_TRANSFER_SRGB` then
  `<<_vkformat,vkFormat>>` must be one of the `+*_SRGB{,_*}+` formats.
  For all other non-YUV formats it must be `KHR_DF_TRANSFER_LINEAR`.
