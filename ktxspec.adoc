...
into 2D array and use <<KTXcubemapIncomplete>> metadata to signal which
faces are present.
...
other combination of parameters makes KTX file invalid.
...
=== Incomplete Cubemap
KTX file can be used to store an incomplete cubemap or an array of 
incomplete cubemaps. In such case, <<numberOfFaces>> must be `1` and 
