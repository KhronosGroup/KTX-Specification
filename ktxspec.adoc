...
After reflation from supercompression or when `supercompressionScheme ==
...
[NOTE]
====
When `<<glInternalFormat>>` is one of the GL_PALETTE* formats from
GL_OES_compressed_paletted_texture <<OESCPT>> this equals the number
of mipmaps and is passed as the levels, parameter when uploading
to OpenGL {,ES}.  However all levels are packed into a single block
of data along with the palette so numberOfMipmapLevels is considered
to be 1 in the for loop over the data. Individual mipmaps are not
identifiable.
====
...
as from the perspective of the KTX2 file there is only a single
level.
...
| 2           | Deflate^1^  | <<DEFLATE>> | n/a
| 4           | Zstandard   |  <<ZSTD>>   | n/a
| 5･･･2^32^-1 | Reserved^2^ |             |
|===

1. Not to be confused with ZLib which is both a compression format and
a library that can be used to compress to ZLib or Deflate formats.

2. A registry will be established to issue values in the reserved range
...
by the reference given in the _Level Data Format_ column of
<<supercompressionSchemes>>.
...
by the reference to the given in the _Global Data Format_ column
...
[TIP]
====
LZW-style lossless supercompression, e.g, schemes 2, 3 and 4, is
generally ineffective on the block-compressed data used by GPU
...
===== Crunch CRN
A file that specifies Crunch CRN with formats other than ETC, ETC2 and
BC[1-5] must be considered invalid.

[WARNING]
====
*IMPORTANT QUESTIONS*

1. Do we want to put such a limitation in KTX2 or are the current
   limitations more to do with lack of CRN encoders rather than with the
   CRN format? If remove limitation don't forget to remove "some" from
   the sentence about CRN above.

2. Any other restrictions we should note about using CRN in KTX2?
====
...
1. Do we need to require any other specific flag values in Zstandard
   frame headers?

2. Should we require a level be a single Zstandard or LZ4 frame or don't care?

3. Any other restrictions we should note about using Zstandard in KTX2?
...
Should we require content checksums anywhere, e.g. on supercompressed data?::
  _Unresolved:_
...
Asciidoctor thinks I am trying to make a list. Same for the Y. below.
////
...
////
- [[[LZ4]]] https://github.com/lz4/lz4/blob/dev/doc/lz4_Frame_format.md[LZ4].
////
...
|      draft5       |  2018-07-26 | Change all size & offset fields to
                                    ...
                                    `cubePadding`.
|     {docrev}      |  {revdate}  | Remove `rowPadding`. Use registered
