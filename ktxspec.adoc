...
`dataCompressionScheme` indicates if an optional compression scheme
has been applied globally to `images`. This is entirely separate
from, and should not be confused with, block compressed texture
formats where the texture in compressed form can be  sampled by the
GPU. When a global compression scheme is used, the image data must
be decompressed prior to GPU sampling. `dataCompressionScheme` must
