...
the values from table 8.11 of the OpenGL 4.4 specification <<OPENGL44>>
...
`valuePadding` fields. The file offset of the `totalImagesSize` field
...
ifdef::totalImagesSize[]
field, and the `totalImagesSize` Size field, is 4 byte aligned.
endif::[]
ifndef::totalImagesSize[]
field is 4 byte aligned.
endif::[]
This padding is included in the `bytesOfKeyValueData` field but not
the individual `keyAndValueByteSize` fields.

ifdef::totalImagesSize[]
=== totalImagesSize
For most textures `totalImagesSize` is the number of bytes of pixel data
in all the LOD levels. It is the sum of all the `imageSize`s plus all
the `mipPadding`s.

The exception is non-array cubemap textures (any texture where
`numberOfFaces` is 6 and `numberOfArrayElements` is 0). For these
textures `totalImamgesSize` = `imagesize` * 6 + sum(`cubePadding`)
+ sum(`mipPadding`).

[TIP]
.Rationale
====
This value is useful for smooth creation of a buffer to hold all
the image data, e.g, a Vulkan staging buffer. Without this field,
applications need either large switch statements to determine size from
`glInternalformat` or `vkFormat` or they will have to read, rewind and
read the file again.
====

endif::totalImagesSize[]
=== imageSize
...
4.4 specification <<OPENGL44>> for a GL_UNPACK_ALIGNMENT of 4.

Values listed in tables and sections referred to in the OpenGL 4.4
<<OPENGL44>> and Vulkan 1.0 <<VULKAN10>> specifications may be
...
ifdef::totalImagesSize[]
Should KTX2 support texture or level sizes > 4GB?::
  _Discussion:_ In other words, should `totalImagesSize` or
  `imageSize` be 64-bit type?
endif::[]
ifndef::totalImagesSize[]
Should KTX2 support level sizes > 4GB?::
  _Discussion:_ In other words, should `imageSize` be 64-bit type?
endif::[]
+
_Unresolved:_
...
Use character or binary values for component swizzles?::
...
What is needed to support tiling?::
  _Discussion:_ Vulkan and recent versions of OpenGL have texture
  tiling. What is needed here to support it?
+
_Unresolved:_
...
- [[[OPENGL44]]] http://www.opengl.org/registry/doc/glspec44.core.pdf[OpenGL^®^ 4.4 Core Profile].
Mark Segal, Kurt Akeley, Jon Leech. The Khronos Group, July 2013.
...
|===
| Version | Date | Remark
| {revnumber} | {revdate} | First incarnation.
|===
