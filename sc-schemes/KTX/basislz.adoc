...
The bitstreams for endpoints, selectors, Huffman tables and the image data are specific to the transcodable format in use. Those for ETC1S are defined in <<BasisLZ Bitstream Specification>>. These bitstreams have to be decoded to reconstruct the base images.
...
The mip level data is a tightly packed array of ETC1S slices indexed by the `ImageDesc` array in the <<basislz_global_data_structure>>.
